...
export function TalentDetail() {
  ...
  useEffect(() => {
    ...
    api.matchesForTalent(id, 5).then((r) => setMatches(r.matches)).catch((e) => toast(`Couldn't load matches: ${(e as Error).message}`, 'error'));
  }, [id]);
        ...
        <section className="md:col-span-2">
          <div className="mb-6">
            ...
            {talent && matches.length > 0 && (
              <LandscapeMap
                focalId={talent.id}
                highlightedIds={matches.map((m) => m.startup.id)}
                height={300}
              />
            )}
          </div>
